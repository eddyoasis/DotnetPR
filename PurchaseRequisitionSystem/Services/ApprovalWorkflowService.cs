...
using PurchaseRequisitionSystem.Data;
...
namespace PurchaseRequisitionSystem.Services
{
    ...
    public class ApprovalWorkflowService : IApprovalWorkflowService
    {
        ...
        public async Task<List<ApprovalWorkflowStep>> GenerateWorkflowStepsAsync(PurchaseRequisition pr)
        {
            ...
            {
                _logger.LogInformation($"⬇️ Fixed Asset < SGD {cfoThresholdSGD:N0}");
            ...
            {
                _logger.LogInformation($"⬆️ Fixed Asset >= SGD {cfoThresholdSGD:N0} - CFO approval REQUIRED");
                steps.Add(new ApprovalWorkflowStep
                {
                    StepOrder = stepOrder++,
                    ApproverRole = "CFO",
                ...
                {
                    _logger.LogInformation($"⬆️ Amount > SGD {ceoThresholdSGD:N0} - CEO approval REQUIRED");
                    steps.Add(new ApprovalWorkflowStep
                    {
                        StepOrder = stepOrder++,
                        ApproverRole = "CEO",
                ...
                {
                    _logger.LogInformation($"⬇️ Amount <= SGD {ceoThresholdSGD:N0} - CEO approval NOT required");
                }
            }
        ...
        public async Task<bool> ProcessApprovalStepAsync(int prId, string approverEmail, ApprovalStatus status, string comments)
        {
            try
            {
                ...
                if (currentStep == null)
                {
                    _logger.LogWarning($"️ No pending step for {approverEmail}");
                    return false;
                }
